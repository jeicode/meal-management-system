...
import { incrementIngredientQuantity, createPurchaseHistory } from '../modules/food-inventory/domain/food-inventory.repository';
import { RabbitMQOrderDatasource } from "../modules/orders/infraestructure/queue-messages/rabbitmq";
import { OrderService } from "../modules/orders/domain/services/orders.service";
...
describe('makePurchase', () => {
  ...
  it('should successfully purchase all ingredients', async () => {
    ...
    expect(orderService.requestOrderHistoryToKitchen).toHaveBeenCalledWith({
      id: 1,
      status: 'PREPARING'
    });
  });
