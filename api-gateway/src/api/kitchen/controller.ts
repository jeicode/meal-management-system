...
export async function getOrdersController(req: Request, res: ServerResponse) {
    try {
        const { take, skip, where, orderBy } = req.body;
        const data = await kitchenService.getKitchenOrders({ take, skip, where, orderBy });
...
export async function kitchenOrderController(req: Request, res: ServerResponse) {
    try {
        ...
        const result = await kitchenService.sendOrderToKitchen({ dishes });
